...
あなたのニックネームは <?php echo $account['Account']['name']; ?> です。

登録メールアドレスは <?php echo $account['Account']['email']; ?> です。

当サイトからの大切なお知らせが正しく届いているかを確認するため、登録メールアドレスの確認を行います。下記URLをクリックしてください。

■登録メールアドレス確認
<?php
echo $html->url(array(Configure::read('Routing.admin') => false, 'action' => 'confirm_register', $account['Account']['email_checkcode']), true);
?>
