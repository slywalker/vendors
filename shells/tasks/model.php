 ...
 * @lastmodified  $Date: 2009-03-09 03:05:18 +0900 (Mon, 09 Mar 2009) $
 ...
 */
	function bake($name, $associations = array(),  $validate = array(), $primaryKey = 'id', $useTable = null, $useDbConfig = 'default') {
		...
		$out .= "\tpublic \$name = '{$name}';\n";

		if ($useDbConfig !== 'default') {
			$out .= "\tpublic \$useDbConfig = '$useDbConfig';\n";
		}
		...
		if (($useTable && $useTable !== Inflector::tableize($name)) || $useTable === false) {
			...
			$out .= "\tpublic \$useTable = $table;\n";
		}

		if ($primaryKey !== 'id') {
			$out .= "\tpublic \$primaryKey = '$primaryKey';\n";
		}

		$validateCount = count($validate);
		if (is_array($validate) && $validateCount > 0) {
			$out .= "\tpublic \$validate = array(\n";
		...
		if (!empty($associations)) {
			...
			if (!empty($associations['hasAndBelongsToMany'])) {
				$out .= "\tpublic \$hasAndBelongsToMany = array(\n";
 ...
 */
	function fixture($model, $useTable = null) {
		...
		$out .= "\tpublic \$name = '$model';\n";

		if (!$useTable) {
			$useTable = Inflector::tableize($model);
		} else {
			$out .= "\tpublic \$table = '$useTable';\n";
		}
		...
		$records = join(",\n", $records);
		$out .= "\tpublic \$records = array(array(\n$records\n\t));\n";
		$out .= "}\n";
